...
export const ForecastScreen: React.FC<ForecastScreenProps> = ({
  ...
  return (
    <div className="space-y-4 animate-in fade-in duration-300">
      ...
      <div className="flex items-center justify-between gap-1 p-1 rounded-2xl bg-slate-100 border border-slate-200">
        {leadTimes.map((lt) => {
      ...
      <div className="grid grid-cols-2 gap-3">
        ...
        >
          ...
          <div className="mt-3">
            <div className="text-[11px] text-slate-500">Rainfall Accum.</div>
        ...
        >
          ...
          <div className="mt-3">
            <div className="text-[11px] text-slate-500">Wind Velocity</div>
            <div className="flex items-baseline gap-1 mt-0.5">
              <span className="text-2xl font-extrabold text-slate-900 font-['Outfit']">
                {parameters.wind.blendedValue}
              </span>
              <span className="text-xs font-bold text-purple-700">km/h</span>
            </div>
            <div className="text-[10px] text-slate-500 mt-1">
      ...
      <div className="glass-card p-4">
        <div className="flex items-center justify-between mb-3">
          ...
          <span className="text-[10px] font-mono text-slate-500">Lead: {leadTime}</span>
        </div>
            ...
            return (
              <div key={m.id} className="space-y-1">
                <div className="flex items-center justify-between text-xs">
                  ...
                  <span className="font-mono font-bold text-slate-900">{val} mm</span>
                </div>
          ...
          <div className="pt-2 border-t border-slate-200 space-y-1">
            <div className="flex items-center justify-between text-xs">
              <div className="flex items-center gap-1.5">
                ...
                <span className="text-sky-800 font-bold font-mono">HYBRIDCAST AI (Blended)</span>
              </div>
              <span className="font-mono font-extrabold text-sky-700 text-sm">{blendedRainfall} mm</span>
            </div>
            <div className="w-full bg-slate-100 rounded-full h-2.5 overflow-hidden p-0.5 border border-sky-300">
              <div
      ...
      <div className="glass-card p-4">
        <div className="flex items-center justify-between mb-3">
          ...
          <span className="text-[10px] text-slate-500 font-mono">Step: 6h</span>
        </div>
        ...
        <div className="grid grid-cols-5 gap-1.5">
          {hourlyTimeline.map((item, idx) => (
            <div
              key={idx}
              className="flex flex-col items-center justify-between p-2 rounded-xl bg-slate-50 border border-slate-200 text-center"
            >
              <span className="text-[10px] font-mono text-slate-500">{item.time}</span>
              <CloudRain className="w-4 h-4 text-sky-600 my-1.5" />
              <span className="text-xs font-bold font-mono text-slate-900">{item.rain}m</span>
              <span className="text-[9px] text-amber-700 font-mono mt-0.5">{item.temp}°C</span>
            </div>
          ))}
        </div>
      </div>
